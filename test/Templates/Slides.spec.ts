...
describe('Slides template', () => {
    ...
    it('renders reveal html', async () => {
        const result = await new template({ resolve: () => Promise.resolve([]) }, '/slides').render();
    ...
    it('matches slides dir with template', async () => {
        ...
        expect(await new template(resolver, 'asdfasdf').render()).to.contain('data-markdown="asdfasdf/01-test.md"');
    });
